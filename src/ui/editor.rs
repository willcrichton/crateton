...
use super::terminal::EditorResources;
...
#[derive(Debug)]
pub struct CodeEditor<'t, S: TextBuffer = String> {
  text: &'t mut S,
  hint_text: String,
  id: Option<Id>,
  id_source: Option<Id>,
...
impl<'t, S: TextBuffer> CodeEditor<'t, S> {
...
impl<'t, S: TextBuffer> CodeEditor<'t, S> {
  pub fn ui(
    ...
    let response = self.content_ui(&mut content_ui, editor_resources);
    let id = response.id;
  ...
  fn content_ui(self, ui: &mut Ui, editor_resources: &EditorResources) -> Response {
    let CodeEditor {
      text,
      hint_text,
      id,
    ...
    if enabled {
      if let Some(pointer_pos) = ui.input().pointer.interact_pos() {
        ...
        if ui.visuals().text_cursor_preview && response.hovered() && ui.input().pointer.is_moving()
        {
          // preview:
      ...
      for event in &ui.input().events {
        let did_mutate_text = match event {
          ...
          Event::CompositionEnd(prediction) => {
            if !prediction.is_empty() && prediction != "\n" && prediction != "\r" && state.has_ime {
              state.has_ime = false;
              let mut ccursor = delete_selected(text, &cursorp);
    ...
    Self::paint_with_highlighting(editor_resources, text, &response, galley, &ui);
    ...
    if response.changed() {
      ...
      let info = WidgetInfo::text_selection_changed(char_range, mask_if_password(text.as_str()));
      response
        .ctx
        .output()
        .events
  ...
  fn paint_with_highlighting(
    ...
    let syntax = editor_resources
      .syntax_set
      .find_syntax_by_extension("py")
      .unwrap();
    let mut h = HighlightLines::new(
      syntax,
      &editor_resources.theme_set.themes["Solarized (dark)"],
    );
    let highlights = LinesWithEndings::from(text.as_str())
      .map(|line| h.highlight(line, &editor_resources.syntax_set));
    let pos = response.rect.min;
    let mut chars = galley.text.chars();
    for (line, hls) in galley.rows.iter().zip(highlights) {
      let mut hls = hls.iter();
      let mut cur_hl = hls.next().unwrap();
      let mut cur_hl_str_iter = cur_hl.1.chars().peekable();
      let line_min_y = pos.y + line.y_min;
      for x_offset in line.x_offsets.iter().take(line.x_offsets.len() - 1) {
        let c = chars.next().unwrap();
        cur_hl_str_iter.next().unwrap();

        let char_pos = Pos2::new(pos.x + x_offset, line_min_y);
        let hl_color = &cur_hl.0.foreground;
        let text_color = Color32::from_rgb(hl_color.r, hl_color.g, hl_color.b);
        ui.painter().text(
          char_pos,
          Align2::LEFT_TOP,
          c,
        ...
        if cur_hl_str_iter.peek().is_none() {
          cur_hl = if let Some(cur_hl) = hls.next() {
            cur_hl
          } else {
            break;
          };
          cur_hl_str_iter = cur_hl.1.chars().peekable();
        }
      }
