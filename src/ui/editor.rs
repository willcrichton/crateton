...
impl<'t> CodeEditor<'t> {
  ...
  pub fn ui(self, ui: &mut Ui, state: &mut State, editor_resources: &EditorResources) -> Response {
    ...
    let response = self.content_ui(&mut content_ui, state, editor_resources);
    let frame_rect = response.rect.expand2(margin);
    let (response2, _) = ui.allocate_painter(frame_rect.size(), Sense::click());
    let response = response | response2;
...
impl<'t> CodeEditor<'t> {
  fn content_ui(
    ...
    let highlights =
      LinesWithEndings::from(text).map(|line| h.highlight(line, &editor_resources.syntax_set));
