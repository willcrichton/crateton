...
fn keyboard_system(
  ...
  mut query: Query<(&mut FlyCamera, &mut RigidBody)>,
) {
  for (mut camera, mut rigid_body) in query.iter_mut() {
    let movement_axis = |plus, minus| {
      if keyboard_input.pressed(plus) {
        -1.0
      } else if keyboard_input.pressed(minus) {
        1.0
      } else {
        0.0
      }
    ...
    let mut fwd_vec = position.rotation.mul_vec3(Vec3::unit_z()).normalize();
    fwd_vec.y = 0.;
    let side_vec = Quat::from_rotation_y(90f32.to_radians())
      .mul_vec3(fwd_vec)
      .safe_normalize();
    ...
    rigid_body.set_position(
      Isometry3::from_parts(position.position + camera.velocity, position.rotation),
      true,
    );
  }
}
...
fn mouse_system(
  ...
  for (mut camera, mut transform) in query.iter_mut() {
    ...
    camera.pitch =
      (camera.pitch + delta.y * camera.sensitivity * time.delta_seconds()).clamp(-89.9, 89.9);
