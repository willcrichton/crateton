...
impl rustc_driver::Callbacks for Callbacks {
  ...
  fn after_analysis<'tcx>(&mut self, _compiler: &Compiler, queries: &'tcx Queries<'tcx>) -> Compilation {
    queries.global_ctxt().unwrap().peek_mut().enter(|tcx| {
      ...
      let jit_module: Module<SimpleJITBackend> = Module::new(jit_builder);
 
      let (_def_ids, cgus) = tcx.collect_and_partition_mono_items(LOCAL_CRATE);
      let mono_items = cgus
        .iter()
...
pub fn setup_scripts() {
  ...
  let externs = package.dependencies.iter()
    .filter(|d| !&["std"].contains(&d.name.as_str()))
    .map(|d| {
  ...
  rustc_driver::catch_with_exit_code(|| {
    let args = format!("rustc scripts/test.rs --edition=2018 -L dependency={} {}", dep_dir, externs);
